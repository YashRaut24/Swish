  ...
  const handleAcceptRequest = async (requesterId) => {
    try {
      ...
      setUser(prev => ({
        ...
        followRequests: (prev.followRequests || []).filter(id => id !== userId)
      }));
    } catch (error) {
      ...
      <div className="bg-white rounded-xl shadow-md p-6">
        <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
          ...
          <div className="flex-1 text-center md:text-left w-full">
            ...
            {user?.eventHostRoles && user.eventHostRoles.filter(hostRole => hostRole.role === "Event Host").length > 0 && (
              <div className="mb-4">
                {user.eventHostRoles.filter(hostRole => hostRole.role === "Event Host").map((hostRole) => (
                  <div key={hostRole.eventId} className="text-sm text-gray-600">
                    Event Host — {hostRole.eventId?.title || 'Unknown Event'}
                  </div>
                ))}
              </div>
            )}
            ...
            {user?.communityRoles && user.communityRoles.length > 0 && (
              <div className="mb-4">
                {user.communityRoles.map((communityRole) => (
                  <div key={communityRole.communityId} className="text-sm text-gray-600">
                    They are {communityRole.role} of {communityRole.communityId?.name || 'Unknown Community'}
                  </div>
                ))}
              </div>
            )}
      ...
      <div className="bg-white rounded-xl shadow-md p-6">
        ...
        {userPosts.length > 0 ? (
          <div className="grid grid-cols-3 gap-1 md:gap-2">
            {userPosts.map((post) => {
              // Check for media (new format) or imageUrl (legacy format)
              const hasImage = (post.media && post.media.length > 0) || post.imageUrl;
