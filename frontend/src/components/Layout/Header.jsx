...
import { Home, Compass, User, LogOut, Menu, Bell, MessageCircle, Users, Search, Calendar } from 'lucide-react';
...
const Header = ({ currentView, onViewChange, onMenuToggle }) => {
  ...
  useEffect(() => {
    loadMessageCounts();
    const interval = setInterval(loadMessageCounts, 30000);
  ...
  const isAdminUser = user?.role === 'admin' || user?.email === 'admin@campus.edu';
  ...
  const navItems = [
    ...(isAdminUser ? [] : [
      ...
      { view: VIEWS.EVENTS, icon: Calendar, label: 'Events' },
      { view: VIEWS.COMMUNITY, icon: Users, label: 'Community' },
      { view: VIEWS.CHAT, icon: MessageCircle, label: 'Messages', badge: unreadMessages },
      { view: VIEWS.NOTIFICATIONS, icon: Bell, label: 'Notifications', badge: unreadCount },
      { view: VIEWS.PROFILE, icon: User, label: 'Profile' },
    ]),
  ];

  if (isFaculty) {
    // Faculty can access all features, no additional nav items needed for now
  }
