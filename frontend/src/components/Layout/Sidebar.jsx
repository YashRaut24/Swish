...
import { Home, Compass, Users, MessageCircle, Bell, User, TrendingUp, Hash, Bookmark, Calendar, Shield } from 'lucide-react';
import { VIEWS } from '../../utils/constants';
import { useAuth } from '../../context/AuthContext';
import { hasRegularNavigationAccess, hasAdminAccess } from '../../utils/roleUtils';

const Sidebar = ({ currentView, onViewChange }) => {
  const { user } = useAuth();

  const mainNavItems = hasAdminAccess(user) ? [
    { view: VIEWS.ADMIN, icon: Shield, label: 'Admin Dashboard', color: 'text-red-600' }
  ] : [
    ...
    { view: VIEWS.EVENTS, icon: Calendar, label: 'Events', color: 'text-teal-600' },
    { view: VIEWS.COMMUNITY, icon: Users, label: 'Communities', color: 'text-purple-600' },
    ...(user?.role === 'Community' ? [{ view: VIEWS.COMMUNITY_DASHBOARD, icon: Users, label: 'Community Dashboard', color: 'text-purple-600' }] : []),
  ...
  return (
    <aside className="hidden lg:block w-64 bg-white border-r border-gray-200 h-[calc(100vh-4rem)] sticky top-16 overflow-y-auto">
      <div className="p-4 space-y-6">
        ...
        {/* Secondary Navigation - Only show for non-admin users */}
        {!hasAdminAccess(user) && (
          <>
            <nav className="space-y-1">
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 px-3">
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (item.label === 'Saved Posts') {
                        onViewChange('saved');
                      } else if (item.label === 'Campus Events') {
                        onViewChange('events');
                      }
                    }}
        ...
        {/* Trending Topics - Only show for non-admin users */}
        {!hasAdminAccess(user) && (
